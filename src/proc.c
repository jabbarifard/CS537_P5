...
mencrypt(char *virtual_addr, int len)
{
  ...
  pdet_t* pgdir = myproc()->pgdir;// pagetable entry//array of pointers to pagetables
  ...
  //
     for (int i=0; i<len; i++) {
	     pte_t* mypte = walkpgdir(pgdir, slider, 0);//each pagetable in pdet
	     if (*mypte & PTE_P == 0) {
		     return -1;
	     }
     }

     slider = PGROUNDDOWN(virtual_addr)
     for(int j=0; j<len; j++){//iterating through 1 pagetable containing many ptes
          pte_t* mypte = walkpgdir(pgdir, slider, 0);//each pagetable in a directory
  ...
  if (uva2ka(pgdir, virtual_addr) == 0) { //len is number of pages I want encrypt
    return -1;
  }
